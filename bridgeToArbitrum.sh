...
forge script ./script/Deployer.s.sol:SetPermissions --rpc-url ${ARB_SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "setAdmin(address,address)" ${ARB_SEPOLIA_REBASE_TOKEN_ADDRESS} ${ARB_SEPOLIA_POOL_ADDRESS}
forge script ./script/Deployer.s.sol:SetPermissions --rpc-url ${ARB_SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "grantRole(address,address)" ${ARB_SEPOLIA_REBASE_TOKEN_ADDRESS} ${ARB_SEPOLIA_POOL_ADDRESS}

# 2. On Sepolia!
echo "Running the script to deploy the contracts on Sepolia..."
output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --verify --etherscan-api-key $ETHERSCAN_API_KEY)
...
forge script ./script/Deployer.s.sol:SetPermissions --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "setAdmin(address,address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${SEPOLIA_POOL_ADDRESS}
forge script ./script/Deployer.s.sol:SetPermissions --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "grantRole(address,address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${SEPOLIA_POOL_ADDRESS}

# Deploy the vault 
echo "Deploying the vault on Sepolia..."
VAULT_ADDRESS=$(forge script ./script/Deployer.s.sol:VaultDeployer --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} --verify --etherscan-api-key $ETHERSCAN_API_KEY | grep 'vault: contract Vault' | awk '{print $NF}')
...
#        address remoteToken
forge script ./script/ConfigurePool.s.sol:ConfigurePool --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "run(address,address,uint64,address)" ${SEPOLIA_POOL_ADDRESS} ${ARB_SEPOLIA_POOL_ADDRESS} ${ARB_SEPOLIA_CHAIN_SELECTOR} ${ARB_SEPOLIA_REBASE_TOKEN_ADDRESS}

# 3.1 Deposit funds to the vault
echo "Depositing funds to the vault on Sepolia..."
cast send ${VAULT_ADDRESS} --value ${AMOUNT} --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD "deposit()"
...
#        uint64 remoteChainSelector,
#        address remoteToken
forge script ./script/ConfigurePool.s.sol:ConfigurePool --rpc-url ${ARB_SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "run(address,address,uint64,address)" ${ARB_SEPOLIA_POOL_ADDRESS} ${SEPOLIA_POOL_ADDRESS} ${SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS}

# 5. Bridge the funds using the script to Arbitrum Sepolia
echo "Bridging the funds using the script to Arbitrum Sepolia..."
SEPOLIA_BALANCE_BEFORE=$(cast balance $(cast wallet address --account sepoliaAccount2 --password $PASSWORD) --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
...
forge script ./script/BridgeTokens.s.sol:BridgeTokens --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --password $PASSWORD --broadcast --sig "run(address,uint64,address,uint256,address,address)" $(cast wallet address --account sepoliaAccount2) ${ARB_SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${AMOUNT} ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
echo "Funds bridged to Arbitrum Sepolia"
SEPOLIA_BALANCE_AFTER=$(cast balance $(cast wallet address --account sepoliaAccount2 --password $PASSWORD) --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
