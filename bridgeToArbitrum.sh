...
# ALLREADY DONE output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${ARB_SEPOLIA_RPC_URL} --account sepoliaAccount2 --broadcast --verify --etherscan-api-key $ARBISCAN_API_KEY)
...
# ALLREADY DONE output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${SEPOLIA_RPC_URL} --account sepoliaAccount2 --broadcast --verify --etherscan-api-key $ETHERSCAN_API_KEY)
